...
def _create_order_with_number(**kwargs) -> Order:
    for _ in range(5):
        try:
            ...
            continue
    raise CheckoutError("Could not allocate a unique order number.")
...
def create_order_from_cart(
    ...
    *,
    ...
    if tax_config is None:
        raise CheckoutError("Tax configuration missing. Run the seed command.")
    ...
    for item in items:
        ...
        # Single source of truth: the same quote call as the preview endpoint.
        quote = quote_product(
            product,
